let oauthPopup = null;
  ...
  // Backend also sanitizes — this is defence in depth.
  function sanitizeInput(val, maxLen) {
    if (!val) return val;
    val = val.trim().replace(/\x00/g, ""); // trim + remove null bytes
  ...
  function show(obj){
    ...
    function stripSlack(t){ return (t||"").replace(/<https?:\/\/[^|>]+\|([^>]+)>/g,"$1").replace(/<https?:\/\/[^>]+>/g,"[link]").replace(/[*_`]/g,""); }
    ...
    // ── DB messages (loadMessagesFromDB) ──────────────────────────────────────
    if (obj.items && Array.isArray(obj.items)){
      ...
      html += section("Messages from DB",
        ...
        + `Showing <strong>${items.length}</strong> messages`
    ...
    // ── search results ────────────────────────────────────────────────────────
      ...
      if (obj.messages.length){
        html += obj.messages.map((m,i) => msgCard(m,i)).join("");
      } else {
        html += `<div style="text-align:center;padding:24px;color:var(--text-muted);font-size:13px;">No messages matched your search.</div>`;
      }
    }

    // ── backfill / join results ───────────────────────────────────────────────
      ...
      if (obj.join && !obj.join_all){
        html += section("Channel Joined", statRow("Channel", obj.join.channel_id||"✓"));
      }
    ...
    // ── chat answer (full AI response in output panel) ────────────────────────
      ...
      if (obj.answer) {
        ...
        var isWarning   = raw.startsWith("⚠️");
        ...
        if (obj.citations && obj.citations.length > 0) {
          function stripSlackText(t){
          ...
          obj.citations.forEach(function(c, i){
            var snippet = esc(stripSlackText(c.snippet || c.text || ""));
  ...
  // Track selections per container independently
  ...
  function selectAllSearchChannels() {
    document.getElementById("multiSearchChannelList").querySelectorAll("input[type=checkbox]").forEach(cb => { cb.checked = true; if(_multiSelections["multiSearchChannelList"]) _multiSelections["multiSearchChannelList"].add(cb.value); });
  }
  function clearAllSearchChannels() {
    document.getElementById("multiSearchChannelList").querySelectorAll("input[type=checkbox]").forEach(cb => { cb.checked = false; if(_multiSelections["multiSearchChannelList"]) _multiSelections["multiSearchChannelList"].delete(cb.value); });
  }
  function selectAllChatChannels() {
    document.getElementById("multiChatChannelList").querySelectorAll("input[type=checkbox]").forEach(cb => { cb.checked = true; if(_multiSelections["multiChatChannelList"]) _multiSelections["multiChatChannelList"].add(cb.value); });
  }
  function clearAllChatChannels() {
    document.getElementById("multiChatChannelList").querySelectorAll("input[type=checkbox]").forEach(cb => { cb.checked = false; if(_multiSelections["multiChatChannelList"]) _multiSelections["multiChatChannelList"].delete(cb.value); });
  }
  ...
  async function searchMessages(){
    try {
      ...
      const q       = sanitizeInput(document.getElementById("keyword").value, 200);
      const from    = document.getElementById("fromDate").value;
      const to      = document.getElementById("toDate").value;
      const user_id = sanitizeInput(document.getElementById("searchUserId").value, 50);
  ...
  async function askChat() {
    try {
      const team_id  = document.getElementById("workspaceSelect").value;
      // Sanitize question before sending — backend also sanitizes (defence in depth)
      const question = sanitizeInput(document.getElementById("chatQuestion").value, 500);
      ...
      if (chatMultiMode) {
        ...
        setStatus("Thinking across " + channelIds.length + " channels" + dateRange + "…");
      } else {
        const channel_id = document.getElementById("channelSelect").value;
      ...
      if (r.status === 429) { setStatus("Too many requests — please wait a moment", "warn"); return; }
      if (r.status === 403) { setStatus("Access denied", "err"); return; }
      if (r.status === 400) { setStatus(data.error || "Invalid request", "err"); return; }
      ...
      if (data.ok && data.answer) {
        ...
        if (data.resolved_username) {
          html = '<div style="font-size:11px;color:var(--teal,#0d9488);margin-bottom:8px;padding:4px 10px;background:rgba(13,148,136,0.07);border-radius:4px;border:1px solid rgba(13,148,136,0.2);display:inline-flex;align-items:center;gap:5px;">'
            + '👤 Filtering messages from <strong>@' + data.resolved_username + '</strong></div><br>' + html;
        }

        const chatFrom = document.getElementById("chatFromDate").value;
        const chatTo   = document.getElementById("chatToDate").value;
        if (chatFrom || chatTo) {
          const rangeLabel = chatFrom && chatTo ? chatFrom + " → " + chatTo
                           : chatFrom ? "From " + chatFrom
                           : "Until " + chatTo;
          html = '<div style="font-size:11px;color:#2563eb;margin-bottom:8px;padding:4px 10px;background:rgba(37,99,235,0.07);border-radius:4px;border:1px solid rgba(37,99,235,0.2);display:inline-flex;align-items:center;gap:5px;">'
            + '📅 Date range: <strong>' + rangeLabel + '</strong></div><br>' + html;
        }
  ...
  function clearSearch(){
    ...
    document.getElementById("chatFromDate").value = "";
    document.getElementById("chatToDate").value = "";
    show({});
    setStatus("Cleared");
  }

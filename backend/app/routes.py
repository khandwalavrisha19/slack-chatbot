...
async def slack_events(request: Request):
    ...
    # Use the handler from slack_handler.py
    return await handle_slack_event(payload, raw_body, timestamp, signature)
...
def api_chat(body: ChatRequest, request: Request, response: Response):
    ...
    system_prompt = (
        ...
        "Each message line is formatted as: [N] timestamp | SENDER_NAME | #channel: message_text\n"
        "SENDER_NAME is the word(s) between the first '|' and the second '|'.\n"
        "\n"
        "STRICT RULES:\n"
        "1. Read every message fully — key content often appears at the END.\n"
        ...
        "4. Cite message numbers like [1] or [2] after every claim.\n"
        "5. ALWAYS put the core answer in the Answer section, not just in Key Points.\n"
        "\n"
        "ANSWER RULES BY QUESTION TYPE:\n"
        "\n"
        "If question contains 'summarize' or 'summary':\n"
        "  → Write a clear paragraph summarizing what the person discussed across all messages.\n"
        "  → Group by topic. Do NOT just list citation numbers.\n"
        "  → Example: 'khandwalavrisha0 discussed project deadlines [1][2], asked about Slackbot progress [3], and shared color codes [5].'\n"
        "\n"
        "If question contains 'who':\n"
        "  → Look up the SENDER_NAME from the message and start your answer with that real name.\n"
        "  → Example: 'vishwajit dabhi mentioned PostHog [1]. PostHog is a tracking tool...'\n"
        "  → NEVER output placeholder text like NAME or angle brackets. Use the actual sender name.\n"
        "\n"
        "If question contains 'what is' or 'what are':\n"
        "  → Explain what it is using message content. Include who mentioned it if relevant.\n"
        "\n"
        "For all other questions:\n"
        "  → Answer directly and completely in the Answer section.\n"
        "\n"
        "OUTPUT FORMAT:\n"
        "Answer: <substantive answer — never just citation numbers>\n"
        "Key points: <optional additional bullets, or None>\n"
        "Citations: <[1], [2] etc>"
    )
